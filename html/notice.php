...
if(isset($_REQUEST["page"])) // $_REQUEST["page"]값이 없을 때에는 1로 지정 
  ...
  if($mode=="search"){
    ...
    $sql="select * from chna03.notice where $find like '%$search%' order by num desc";
  } else {
    $sql="select * from chna03.notice order by num desc, ord asc limit $first_num, $scale";
    //limit를 사용해 레코드 개수를 한 페이지당 출력하는 수로 제한
  }
  try{  
    $stmh = $pdo->query($sql); 

$sql = "select * from chna03.notice";  //전체 레코드수를 파악하기 위함.
$stmh1 = $pdo->query($sql);
...
<!--
  ...
  while($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
    ...
    <div id="list_item">
      ...
      <div id="list_item2"><?=$space?><a href="notice_view.php?num=<?=$item_num?>&page=<?=$page?>"><?= $item_subject ?></a></div>
  ...
  <div id="page_num">
    ...
    if($page!=1 && $page>$page_scale)
    {
     ...
     if($page<$total_page)
     {
      ...
      print "<a href=notice.php?page=$next_page> ▶</a><p>";
    }
    ?>      
  </div>
</div>
...
<div id="write_button">
  ...
  {
    ?>
    <a href="notice_write_form"><img src="./img/write.png"></a>
    <?php
  }
  ?>
</div>
