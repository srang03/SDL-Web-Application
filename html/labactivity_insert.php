...
$content=htmlspecialchars($_REQUEST["content"]);
 ...
 if ($mode=="modify"){
   ...
   $position = $_REQUEST['del_file'];
    ...
    try{
        $sql = "select * from chna03.labactivity where num=?";  // get target record
      ...
      for ($i=0; $i<$count; $i++)                  
      {
       ...
       if ($del_ok[$i] == "y")
       {
        ...
        try{
         ...
         $sql = "update chna03.labactivity set $field_org_name = ?, $field_real_name = ?  where num=?";
         $stmh = $pdo->prepare($sql); 
...
try{
  $pdo->beginTransaction();   
  $sql = "update chna03.labactivity set subject=?, content=?, is_html=? where num=?";
  $stmh = $pdo->prepare($sql); 
  ...
  header("Location:http://chna03.dothome.co.kr/labactivity");
 ...
 {
   try{
       $sql = "select * from chna03.labactivity where num = $num"; // 부모 글 가져오기
       $stmh = $pdo->prepare($sql);  
       $stmh->bindValue(1, $num, PDO::PARAM_STR);      
       $stmh->execute();            
 ...
 // ord 가 부모글의 ord($row[ord]) 보다 큰 경우엔 ord 값 1 증가 시킴
       $pdo->beginTransaction();
       $sql = "update chna03.labactivity set ord = ord + 1 where group_num = ? and ord > ?";
       $stmh = $pdo->prepare($sql); 
       ...
       $pdo->beginTransaction();
       $sql = "insert into chna03.labactivity(id, name, subject, content, regist_day, hit, is_html,";
       $sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2 , group_num,depth,ord) ";
       $sql .= "values(?, ?, ?, ?, now(), 0, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";
       $stmh = $pdo->prepare($sql); 
       ...
       $stmh->bindValue(12, $group_num, PDO::PARAM_STR);  
       $stmh->bindValue(13, $depth, PDO::PARAM_STR);  
       $stmh->bindValue(14, $ord, PDO::PARAM_STR);   
      ...
      try{
       ...
       $sql = "insert into chna03.labactivity(id, name, subject, content, regist_day, hit, is_html,";
       $sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2 , depth,ord) ";
       $sql .= "values(?, ?, ?, ?, now(), 0, ?, ?, ?, ?, ?, ?, ?,?,?)";
       $stmh = $pdo->prepare($sql); 
       $stmh->bindValue(1, $_SESSION["userid"], PDO::PARAM_STR);  
       $stmh->bindValue(2, $_SESSION["name"], PDO::PARAM_STR);  
       ...
       $stmh->bindValue(10, $copied_file_name[1], PDO::PARAM_STR);  
       $stmh->bindValue(11, $copied_file_name[2], PDO::PARAM_STR);
       $stmh->bindValue(12, $depth, PDO::PARAM_STR);  
       $stmh->bindValue(13, $ord, PDO::PARAM_STR);   
       ...
       $pdo->beginTransaction();   
       $sql = "update chna03.labactivity set group_num = ? where num=?";
